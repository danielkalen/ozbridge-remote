...
| `oz_agent_run_cloud` | cloud | Launch a cloud Oz agent. **Consumes Warp credits.** **`model` and `environment` are both required.** Returns a run id immediately; the run itself takes a few minutes to complete. Fetch the final result with `oz_run_result` (preferred over `oz_run_get`, which returns the full raw payload and truncates output) or check state with `oz_run_status` — either poll at intervals or stop and re-run a fetch manually once the run has had time to finish. Every successful response includes a reminder of this. Params: `prompt` (required), `model` (**required**), `environment` (**required**), `skill`. |
| `oz_run_get` | read-only | Fetch the full raw run payload by id (status + output); may truncate long output. Prefer `oz_run_result` for the agent's final result and `oz_run_status` for a status-only subset. Params: `runId` (required). |
...
| `oz_run_result` | read-only | The assistant's trailing final result: trailing run of `role="assistant"` text entries joined by blank lines, falling back to `status_message.message`, else `""`. **Preferred over `oz_run_get` for retrieving an agent's final output.** Params: `runId` (required). Also exposed as `GET /agent/runs/:runId/result`. |
...
- **Cloud runs are asynchronous.** `oz_agent_run_cloud` returns a run id immediately and the agent run takes a few minutes to complete in the cloud environment. Fetch the final result with `oz_run_result` (preferred over `oz_run_get`, which returns the full raw payload and truncates output) or check state with `oz_run_status` — either poll at intervals or stop and re-run a fetch manually. Every successful `oz_agent_run_cloud` response appends a guidance block reminding the caller of this.
